...
async function tradeCommodity(trade) {
    ...
    let tradeNotification = getFactory().newEvent('org.example.biznet', 'TradeNotification');
...
async function removeHighQuantityCommodities(remove) {
    ...
    for (let n = 0; n < results.length; n++) {
        ...
        let removeNotification = getFactory().newEvent('org.example.biznet','RemoveNotification');
